<template>
  <div>
    ...
    >
      <b-card-text>
        Your model is created successfully. Upload photo.
      </b-card-text>
    </b-modal>
  </div>
</template>
  ...
  mounted() {
    useJwt.getModelDetail(router.currentRoute.params.model_id)
      .then(response => {
        ...
        this.county.label = this.userDataInfo.county
      }).catch(error => {
    ...
    submit() {
      this.$refs.createModelForm.validate().then(success => {
        if (success) {
          ...
          useJwt.updateModel(router.currentRoute.params.model_id, this.userDataInfo)
            .then(response => {
